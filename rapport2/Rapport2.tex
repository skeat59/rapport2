...
\chapter{Les nouveautés}
	\section{Le Réseau}
		Pouvoir jouer seul en débloquant des niveau, jouer dans un mode infini et repousser nos limites et même envoyer notre score en ligne sont des possibilités intéressantes. Mais à part comparer son score, on se sens vite seul, et on peux se lasser du jeux.\\
		
		C'est pourquoi nous avons choisit, pour votre plus grand plaisir, d'apporter une "légère" nouveauté à CandyBird : Un mode réseau.\\
		...
		Maintenant que vous êtes connecté au server, le votre ou celui d'un ami, ce que vous souhaiter,  c'est pouvoir jouer ! Et bien il suffit simplement de lancer le mode infini et vous verrez votre/vos amis apparaitre sur la map à vos cotés.\\
		
		Le but du réseau n'est pas d'arriver avant les autres au bout de la map. Vous pouvez toujours essayer mais le mode infini n'as pas de fin donc vous essaierez longtemps.. De plus, la map défile à la même vitesse chez toutes les personnes connecté au même server, et les blocs s'enchainent de la même façon. Vous devez donc survivre aussi longtemps que les autres, et être le dernier en vie pour faire le meilleur score. Tout comme donc le mode infini normal, votre score est envoyer sur notre site en cas de défaite, afin que votre meilleur score soit sauvegardé.
		
		\newpage
		
		\subsection{Comment ça marche en coulisse ?}
		...
		A chaque Update du Game1, les Clients envoie leur pseudo ainsi que leur position (sur x et sur y). De même, ils récupèrent les données envoyées par les autres Clients afin de mettre à jour leur position.
